...
\begin{frame}
  ...
  \begin{itemize}
    \item<1-> In distributed protocols, generating key material without relying on a trusted entity is often desirable.
    \item[]
...
\begin{frame}
  ...
  \end{itemize}
    \item[]
    \item<6-> After having received all other party's commitments, each party publishes $X_i$
    \item[]
    \item<7-> Each party checks that $c_i \iseq H(X_i)$. If not, they abort and identify misbehaving parties.
    \item[]
    \item<8-> Otherwise, $\sk \gets \sum x_i$  and $\pk \gets \prod X_i$.
  \end{itemize}
\end{frame}
...
\node at (0,0) {
\scalebox{0.9} {
  \procedure{GJKR Construction: Rounods One and Two}{
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item<2-> Adversary controls the set $\corrupt$ of $t-1$ parties, environment controls $\honest$ where $\lvert \honest \rvert \geq t$.
    \item<3-> Follow round one and two honestly for all simulated parties $\honest$.
    \item<4-> Because the environment controls $t$ parties, it can learn $\secretone_i, i \in \corrupt$.
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item In round three, for one honest party $\ell$, set:
  \begin{itemize}
    ...
    \item<3-> Derive the commitment to the coefficients  relative to the shares published in round one
