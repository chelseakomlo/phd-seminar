...
%
  \uncover<7-> { \qquad \qquad \qquad \sendmessageright{top=\text{ {\bf Broadcast}  $\vsscommitthree_i $   }}  \< \\}
  \uncover<8-> { \qquad \qquad \qquad  \sendmessageright{top=\text{ {\bf Send} $\shareone_{ij},  \sharetwo_{ij}$ to player $j$   }}  \< \\ }
  \uncover<9-> { \text{For all i where } \shamir.\verify(\shareone_{ji}, \sharetwo_{ji}, \vsscommitthree_k) = 1:  \> \\ }
  \uncover<10-> { \qquad \qquad  \qual \gets \qual \bigcup i  \> \\ }
}
}
};
\end{tikzpicture}
...
\scalebox{0.9} {
  ...
  \uncover<2-> { C = 0 \> \\[1mm] }
  \uncover<3-> { \text{For all $j$ where } \shamir.\verify(\shareone_{ki}, \vsscommitone_k) \neq 1:  \> \\[1mm] }
  \uncover<4-> { \qquad \qquad C \gets C + \shamir.\recover(\{ \shareone_{kj} \}_{j \in C} )    \> \\[1mm] }
%
  \uncover<5-> { \sk_i = C + \sum_{k \in \qual} \shareone_k     \\[1mm] }
