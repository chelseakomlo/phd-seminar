...
%
  ...
  \uncover<9-> { \text{if } \shamir.\verify(\shareone_{ji}, \sharetwo_{ji}, \vsscommitthree_k) = 1:\ \qual \gets \qual\ \bigcup\ \{i\}  \> \\ }
}
}
};
\end{tikzpicture}
...
\scalebox{0.9} {
  \procedure{GJKR Construction: Round Three}{
    \textbf{\underline{Participant i}}  \> \textbf{\underline{Other Participants}}  \\
  \uncover<1-> { \qquad \qquad \qquad \sendmessageright{top=\text{ {\bf Broadcast}  $\vsscommitone_i $   }}  \< \\[1mm]}
  \uncover<2-> { \gamma = 0;\ \qual' \gets \qual \> \\[1mm] }
  \uncover<3-> { \text{For all $j$ where } \shamir.\verify(\shareone_{ji}, \vsscommitone_j) \neq 1:  \> \\[1mm] }
  \uncover<4-> { \qquad \qquad \gamma \gets \gamma + \shamir.\recover(\{ \shareone_{ji} \}_{j \in C} )   \> \\[1mm] }
  \uncover<4-> { \qquad \qquad  \qual' \gets \qual' \setminus \{ j \}   \> \\[1mm] }
%
  \uncover<5-> { \sk_i = \gamma + \sum_{k \in \qual'} \shareone_k     \\[1mm] }
  \uncover<6-> {  \pk \gets  \prod_{k \in \qual} A_{k0}  = g^{\sum_{i \in \qual} \secretone_i  }    \> \\[1mm] }
}
}
};
\end{tikzpicture}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Any cheating party must choose their commitmentments $\vsscommitone_A, \vsscommittwo_A,  \vsscommitthree_A$ having seen only $\vsscommitthree_i, \ldots$ from other players.
    \item[]
    \item<2-> Pedersen commitments guarantee that $g^ih^j$ will not reveal any information about $g^i$.
    \item[]
    \item<3-> If a player disqualifies themselves in round $3$ (after learning $\vsscommitone_i, \ldots$), $\secretone_A$ can be extracted (assuming $t$ honest parties).
    \item[]
    \item<4-> So even if player $A$ is disqualified, the public key will remain unbiased even though $\vsscommitone_i, \ldots$ has been published in the clear.
  \end{itemize}
\end{frame}
