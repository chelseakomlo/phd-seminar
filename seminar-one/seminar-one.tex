...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item<2-> Adversary controls $t-1$ parties $\corrupt$, environment controls $\honest$ where $\lvert \honest \rvert \geq t$.
    ...
    \item<5-> In round three, for one honest party $\ell$, set:
  \begin{itemize}
    \item<6-> $A_{\ell 0} \gets \dkgchal \cdot g^{\sum \secretone_i},\ i \in \honest \cup \corrupt \setminus \{ \ell\}$
  \end{itemize}
