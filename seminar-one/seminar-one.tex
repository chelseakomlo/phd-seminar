...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item<3-> Goal is to:
  \begin{itemize}
    \item<4-> Generate a secret that all $n$ parties contribute to but no party knows
...
\begin{frame}
  \frametitle{Pedersen DKG}

  \begin{itemize}
      \item<1-> Each participant $i$ acts as the dealer and performs a Shamir secret sharing of a secret $\secretone_i$.
    \item[]
      \item<2-> The group's secret at the end is $\sk \gets \sum \secretone_i$
    \item[]
      \item<3-> GJKR showed that a rushing adversary can bias key material, but by a limited amount.
    \item[]
      \item<4-> Proven secure in the context of threshold signatures.
  \footnote{
...
\scalebox{0.9} {
  ...
  \uncover<7-> { \sk_i = \sum_{k \in \qual} \shareone_k \lambda_k,\ \ \text{$\qual$ are the players that issued valid shares. }    \\ }
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item[]
    \item<5-> Proven secure in a stand-alone manner.
  \end{itemize}
\end{frame}
